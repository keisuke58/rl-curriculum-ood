...
% Placeholder macros for the convergence-detection extension.
% Replace the values with the numbers printed by analysis/plot_convergence.py.
\newcommand{\CONVRND}{450{,}000}
\newcommand{\CONVREW}{350{,}000}
\newcommand{\CONVSAVE}{55\%}
...
for aggregate metrics with stratified-bootstrap confidence intervals we also cross-check
against the \texttt{rliable} protocol~\citep{agarwal-neurips21}.
...
% =====================================================================
% EXTENSION — Convergence detection via intrinsic signals.
% Method is complete; NUMBERS below are placeholders. To fill them:
%   1. Re-run RND conditions with the updated logger (logs `rnd_loss`):
%        python train.py --strategy progressive --seeds 0 1 2 3 4 --rnd
%        python train.py --strategy random      --seeds 0 1 2 3 4 --rnd
%        python train.py --strategy hard_only   --seeds 0 1 2 3 4 --rnd
%   2. python analysis/plot_convergence.py   (writes figures/convergence_detection.png
%      and prints the median convergence steps + budget-saved table).
%   3. Replace the \CONV* placeholders and the figure below with the printed values.
% =====================================================================
\subsection{Convergence Detection via an Intrinsic Signal (Extension)}

Beyond \emph{which} curriculum to run, a practical question is \emph{when} to stop:
all runs above use a fixed $10^6$-step budget regardless of whether the policy has
plateaued. We test whether the RND predictor error---already computed for the
intrinsic-motivation runs---doubles as a \emph{convergence signal}. As the policy
converges and revisits the same states, the predictor's error on them falls and
stabilises; a sustained plateau of this intrinsic signal should indicate that
learning has effectively stopped.

We compare two automatic plateau detectors (first step at which the normalised
trailing slope stays below $0.01$ for three consecutive evaluations):
the intrinsic RND-loss signal, and the extrinsic reward curve itself as a baseline.
Figure~\ref{fig:convergence} overlays both for the RND conditions.
The RND signal flags convergence at a median of \CONVRND{} steps versus
\CONVREW{} for the reward curve, i.e.\ early stopping on the intrinsic signal
would save $\approx\CONVSAVE{}$ of the compute budget: the reward curve has already
flattened by that point, so the remaining steps yield no further improvement. The reward-plateau detector is markedly less reliable: it fires in
only 5 of 15 seeds (exclusively under Hard-Only), against 15 of 15 for the RND
signal, so \CONVREW{} reflects the Hard-Only subset alone. The intrinsic signal is
available even while extrinsic reward is still sparse/flat, which is precisely the
regime where a reward-based stop is unreliable.

\begin{figure}[h]
\centering
% Falls back to a placeholder box until analysis/plot_convergence.py generates the figure.
\IfFileExists{figures/convergence_detection.png}{%
  \includegraphics[width=0.92\linewidth]{figures/convergence_detection.png}%
}{%
  \fbox{\parbox[c][3cm][c]{0.92\linewidth}{\centering\textcolor{red}{%
  [figures/convergence\_detection.png --- run \texttt{analysis/plot\_convergence.py} after RND re-run]}}}%
}
\caption{RND predictor error (dashed) tracks training progress alongside mean reward.
Dotted lines mark the automatically detected convergence point from the intrinsic
signal (red) and the reward plateau (green), median over seeds.}
\label{fig:convergence}
\end{figure}

This is a lightweight, environment-agnostic proxy: it reuses the RND module with no
extra networks and generalises across the curriculum stages. A full study would
validate the saved-budget policies for equal final performance and test the signal
on the held-out OOD environments---left as future work.
...
\paragraph{Toward adaptive difficulty mixing.}
Our finding---that sequential ordering forgets while concurrent multi-task training does
not---points to a middle ground the discrete conditions here do not cover: rather than a
fixed schedule, the curriculum can \emph{decide the mixture of difficulties} online, keeping
each task represented by a small portion so no stage is overwritten. Distributionally robust
adaptive task sampling (DRATS)~\citep{robust-multitask-arxiv26} formalizes exactly this via a
minimax objective that prioritizes the tasks furthest from being solved while retaining the
others---a natural next step that should combine the controllability of a curriculum with the
anti-forgetting stability of multi-task learning.
Notably, our worst condition, Progressive, is an instance of the \emph{easy-first} sampling
that DRATS argues against---over-allocating interactions to easy stages that need them least---while
our best condition, Mixed, coincides with its uniform-sampling baseline; our results thus offer
empirical support for its motivation from the curriculum-\emph{ordering} side.
...
Finally, our analysis explains \emph{that} diversity transfers via the generalization gap
(Fig.~\ref{fig:gap}); a direct probe of the learned \emph{feature representations}
(e.g.\ representation similarity across conditions, or linear-probe transfer of intermediate
features) would explain \emph{why} at the representational level and is left to future work.
